...
阶段 10：TileLang 到 Golem SST E2E 与 extractor 扩展收尾。当前已完成真实 TileLang -> `CIM-TileIR` -> Golem SST -> stats -> report 端到端 smoke，以及参数化 TileLang GEMM 源码生成入口；同时完成 softmax 阶段一：`CIM-TileIR` 可表达 row-wise softmax 和 `matmul -> softmax` graph IR，但 Golem exporter 仍只接受 GEMM，确保当前 GEMM E2E 不被破坏。阶段 11 runtime ABI / ELF 闭环仍是长期目标。
...
- [x] 新增 `build_softmax_ir()`，支持编译器侧 row-wise softmax IR 构造与校验
- [x] 新增 `build_matmul_softmax_graph_ir()`，支持 `matmul -> softmax` 两节点 graph IR 构造与校验
- [x] 保持 Golem exporter 只接受 `kernel=gemm`，对 softmax/graph IR 明确拒绝，避免破坏现有 GEMM E2E
