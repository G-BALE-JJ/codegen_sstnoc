...
### Softmax 阶段一：仅扩展 CIM-TileIR 表达，不破坏 GEMM E2E
- **状态：**完成
- 背景：
  - 用户确认下一步先执行阶段一：让当前 IR 支持 softmax，但要求不破坏现有自定义优化 GEMM E2E。
  - 本阶段只修改 `codegen_sstnoc` 编译器侧 IR/checker/API，不修改 `/data4/jjgong/RISC-V-CIM-Manycore-SST`。
- 已执行的操作：
  - 新增 `build_softmax_ir()`，可构造 row-wise softmax 的 `CIM-TileIR`：
    - 输入/输出均为 2D row-major tensor。
    - 当前仅支持 `axis=1`。
    - program 形态为 `row_max -> subtract -> exp -> row_sum -> divide -> store`。
  - 新增 `build_matmul_softmax_graph_ir()`，可表达 `matmul -> softmax` 两节点 graph IR：
    - `A, B -> S` 为 matmul。
    - `S -> P` 为 row-wise softmax。
    - graph 阶段只作为 IR 表达和 JSON 导出，不进入 Golem SST exporter。
  - 扩展 `validate_cim_tile_ir()`：
    - 保持原有 GEMM 校验路径不变。
    - 新增 `kernel=softmax` 校验。
    - 新增 `kernel=graph` 的当前 MVP 校验，只接受 `matmul, softmax` 顺序。
  - 导出新 API：
    - `build_softmax_ir`
    - `build_matmul_softmax_graph_ir`
  - 新增测试覆盖：
    - softmax IR 可校验、可 JSON 导出。
    - softmax axis/shape 错误会被 checker 拒绝。
    - `matmul -> softmax` graph IR 可校验、可 JSON 导出。
    - Golem exporter 对 softmax/graph IR 明确拒绝。
    - 原 GEMM Golem exporter 路径仍可正常生成 matmul descriptor。
- 已创建/修改的文件：
  - `tilelang_cim/builder.py`
  - `tilelang_cim/checker.py`
  - `tilelang_cim/__init__.py`
...
```bash
python -m pytest tests/test_cim_tile_ir.py tests/test_golem_exporter.py -q
bash scripts/check_docs.sh
TILELANG_CACHE_DIR=/data4/jjgong/tmp/tilelang-cache python -m pytest tests -q
```

- 验证结果：
  - 目标测试：`14 passed in 0.27s`。
  - 文档检查：基础文档检查通过。
  - 完整 pytest：`60 passed in 36.20s`。
- 当前边界：
  - softmax 目前只在 `CIM-TileIR` 层表达，不导出 Golem SST artifacts。
  - Golem exporter 仍只接受 `kernel=gemm`，以保护当前已跑通的 GEMM E2E。
  - 下一阶段若要真实 SST execute softmax，需要在 `RISC-V-CIM-Manycore-SST` 中新增 softmax runtime/contract 或 graph runtime。
